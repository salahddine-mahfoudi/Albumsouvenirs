    ...
    return (
        <Paper className={classes.paper}>
            <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
                <Typography variant="h6">{ currentId ? 'Editer' : 'Créer' } un souvenir</Typography>
                <TextField name="creator" variant="outlined" label="Createur" fullWidth value={postData.creator} onChange={(e) => setPostData({ ...postData, creator: e.target.value })}/>
                ...
                <Button variant="contained" color="secondary" size="small" onClick={clear} fullWidth>rafraichir</Button>
            </form>  
        </Paper>
